...
@WebServlet(urlPatterns = {"/createCBL"})
@MultipartConfig
public class CreateCanBoLopServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		RequestDispatcher rd = request.getServletContext()
				.getRequestDispatcher("/views/createCBLView.jsp");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
	   ...
	   boolean isCanBo = banCanSuLopDAO.checkStudentAsClassLeader(sinhVien.getMaSV(), ngayBatDau, ngayKetThuc);
	   if(isCanBo) {
		   errors.add("sinh viên đã có vai trò khac");
	   }
	    if (errors.isEmpty()) {
	        BanCanSuLop cn = new BanCanSuLop(lop, sinhVien, ngayBatDau, ngayKetThuc, nienKhoa,role);

	        boolean insertResult = banCanSuLopDAO.insertBanCanSuLop(cn);
	        if (!insertResult) {
	            errors.add("Thêm Ban cán sự không thành công");
	        } else {
